...
const char *SERVER_URL = "http://192.168.8.142:3000/event"; // Pi IP - change if different
...
void loop()
{
  ...
  for (int i = 0; i < 7; i++)
  {
    ...
    if (slotState[i] && !lastSlotState[i])
      pendingSend[i] = true; // send only on press
...
void retryPending()
{
  for (int i = 0; i < 7; i++)
  {
    if (pendingSend[i])
    {
      if (sendCrystalEvent(i))
        pendingSend[i] = false;
    }
  }
}

bool sendCrystalEvent(int slot)
{
  HTTPClient http;
  ...
  String payload = "{ \"type\": \"crystal\", \"slot\": " + String(slot) + " }";
  ...
  {
    drawStatus("Err " + String(code), crystalCount);
  }
  return (code == 200);
}
